...
\begin{abstract}
	...
	We show that a multi-species model can improve precision of detectability estimates for species with low sample size, and even with species up to 1000 observations.
	However, when there are no data available for a species, the multi-species models presented here tend to be overly conservative in predicting detectability. 
...
\par We generated a phylogenetic correlation matrix $\mathbf{C}$ using the same methods outlined in the Appendix of \citet{solymos_phylogeny_2018} that generate distances between species, and further processed using the R package "ape" \citet{paradis_ape_2019}.
...
However, two out of the 315 species had a percent change in EDR of more than 10\%: Greater Prairie-chicken (\textit{Tympanuchus cupido}) and Chihuahuan Raven (\textit{Corvus cryptoleucus}) (Figure \ref{fig:1vs1}B).
...
Among the two species with greater than 10\% change in EDR, Chihuahuan Raven had the single-species model chosen as the preferred model, and Greater Prairie-chicken had no preference.
...
That is, in the absence of any data (or with very little data in the case of Kirtland’s Warbler), the estimated EDR of Bicknell’s Thrush and Kirtland’s Warbler was similar to those of other Thrushes and Warblers, respectively, but with small departures from overall mean based on specific traits.
...
\par For Kirtland's Warbler, Equations 1 and 2, the predicted cue rate of 0.29, predicted EDR of 75.3 m, and an example survey design of 5 minutes recording all birds within 100 m, we estimate a detectability of 0.36.
...
\par In this paper, we developed a multi-species model of detectability that accounts for phylogeny and traits, expanding upon the work from \citet{solymos_phylogeny_2018}.
...
That is, we did not see too many significant changes in cue rate and EDR by modelling with a multi-species model versus modelling with a single-species model.
This provided an appropriate baseline to continue our comparisons for data-sparse species.
We saw a few exceptions in the distance modelling, in that estimates of EDR for Greater Prairie-chicken and Chihuahuan Raven in a multi-species context were much lower than that estimated from a single-species context.
We note that these species are generally observed either in flight in highly specific regions (for Chihuahuan Raven) or in wide-open landscapes (for both). 
Thus, it is possible that data for these species are biased high, such that a multi-species model brings the estimates down to a more realistic value.

\par We did not see a large difference in precision of estimates between the single-species and multi-species model, except for a slight difference in cue rate estimates for sample sizes less than 1000.
...
Therefore, while the multi-species implementation can somewhere improve precision of estimates for lower sample size, the use of even weakly-informed priors clearly goes a long way in improving precision.
...
\par Interestingly, these multi-species models were not able to provide good estimates of cue rate or EDR for species where we had no data, as evidenced by our leave-species-out cross validation.
...
Future studies could seek to disentangle the possible effects of habitat on cue rate; to go one step further, future studies should build upon the work of \citet{martin-schwarze_joint_2021} to assess how truly independent cue rate and EDR are.
...
When investigating further, Figure \ref{fig:distance_cv_traits} demonstrated a fairly clean deliniation of these different trait groupings, with little variation within each.
...
Future studies should seek to continue understanding drivers and predictors of detection distance in birds, both related to and complementary to traits.

\subsection{Opportunities to Expand Estimates of Detectability in North American Landbirds}

\par Given the results of the k-fold cross-validation, we were comfortable with applying this model to species where we had only a few data points, such as Harris's Sparrow, Lesser Prairie-chicken, Spotted Owl, LeConte's Thrasher, and Tricolored Blackbird.
...
\par We caution, however, the use of these models for species with no data points, given the result of the leave-species-out cross validation.
For Bicknell's Thrush and Kirtland's Warbler, both estimates of detectability appeared to be underestimated compared to estimates found in the literature, which corroborates the consistent underestimation found from the cross-validation.
Of course, in the absence of any detectability information at all, our models can at least provide a starting point for a researcher to consider, which can then be turned into ranges of potential detectability \citep{bennett_how_2024}.
...
In both cases above, we also caution that detectability predictions for species with few data (e.g., < 1000 observations) should be identified as such, so that users know that the estimate depends on data from other similar species.
...
For applications that tend to rely on known presences, such as threat assessments, our method opens new possibilities for incorporating detectability, and thus gaining a fuller understanding of the potential for undetected occurrences. 
